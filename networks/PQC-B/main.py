    ...
    for comm_round in range(latest_round_num + 1, args['max_num_comm'] + 1):
        ...
        for miner_iter in range(len(miners_this_round)):
            ...
            if final_transactions_arrival_queue:
                ...
                for (arrival_time, unconfirmmed_transaction) in final_transactions_arrival_queue:
                    if miner.is_online():
                        ...
                        verification_time, is_validator_sig_valid = miner.verify_validator_transaction(
                            unconfirmmed_transaction, 3 * rewards)
                        if verification_time:
                            if is_validator_sig_valid:
                                validator_info_this_tx = {
                                    ...
                                    'miner_rewards_for_this_tx': 3 * rewards}
                ...
                if miner.is_online():
                    ...
                    with open(f"{bench_folder}/block_gen_1.txt", "a") as file:
                        file.write(
                            f"State Time Point: {start_time_point}, Block Generation Time Spent {block_generation_time_spent}\n")
        ...
        for miner in miners_this_round:
            if not miner.is_malicious:
                difference_value = abs(ratio - miner.return_vrf_output())
        ...
        for device in devices_list:
            ...
            if the_added_block:
                ...
                with open(f"{bench_folder}/block_gen_time.txt", "a") as file:
                    file.write(
                        f"COMM ROUND: {comm_round} , {device.idx} - Block Gen Time Point: {block_generation_time_point} : COMM Block Gen Time: {comm_round_block_gen_time}\n")
