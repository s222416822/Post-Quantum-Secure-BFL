...
class Signature(ct.Structure):
    ...
    def sign(self, message):
        ...
        """
        # print("1SIGN Message--OQS", message)
        message = str(message).encode()
        # print("2SIGN Message--OQS", message)

        # provide length to avoid extra null char
        my_message = ct.create_string_buffer(message, len(message))

        # my_message = ct.create_string_buffer(message, len(message)) #added by D

        message_len = ct.c_int(len(my_message))

        # message_len = ct.c_int(len(str(my_message)))  #changes made by D
    ...
    def verify(self, message, signature, public_key):
        ...
        # Custom added by D]
        # print("1Message--OQS", message)
        message = str(message).encode()
        # print("2Message--OQS", message)
