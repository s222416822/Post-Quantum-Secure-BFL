...
def vrf_run(alpha_string):
    ...
    # secret_key = keyPair.d
    # public_key = keyPair.e
    # print(secret_key)
    # print(public_key)
    ...
    secret_key = wotsplus.privkey
    # print(type(secret_key))
    secret_key = str(secret_key).encode("utf-8")
    # print(secret_key)
    public_key = wotsplus.pubkey
    # print(public_key)

    # public_key = str(public_key).encode("utf-8")

    # public_key = str(secret_key).encode("utf-8")
    public_key = get_public_key(secret_key)
    # print("Public key", public_key)

    # https: // research.nccgroup.com / 2020 / 02 / 24 / reviewing - verifiable - random - functions /
    # Alice generates a beta_string commitment to share with Bob
    # alpha_string = b'Hello'
    # pi_string - intermediate proof, initially private, save it for future use
    p_status, pi_string, signature = ecvrf_prove(secret_key, alpha_string, wotsplus)
    # print("P Status", p_status)
    # print("Pi String", pi_string)

    # Run proof to hash takes pi_string and returns final digest beta_string to share with Bob
    # pi string used to calculate and submit beta string as our sealed auction bid
    b_status, beta_string = ecvrf_proof_to_hash(pi_string)
    # print("B Status", b_status)
    print(type(beta_string))
    print("Beta String", beta_string)
    # print("Beta String", beta_string.decode("'unicode_escape'"))
    ...
    # to get locally calculated beta_string2 to compare with initial beta_string
    result, beta_string2 = ecvrf_verify(public_key, pi_string, alpha_string, signature, wotsplus)
    # print("Result:", result)
