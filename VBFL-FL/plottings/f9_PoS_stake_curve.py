...
for log_var_iter in range(len(vars_names)):
	...
	for device_idx, stakes in vars()[f'{log_var}_stakes'].items():
		if device_idx.split(' ')[-1] == 'M':
			axs[log_var_iter].plot(range(draw_comm_rounds), [_/10 for _ in stakes], color='red')
		else:
			axs[log_var_iter].plot(range(draw_comm_rounds), [_/10 for _ in stakes], color='green')
