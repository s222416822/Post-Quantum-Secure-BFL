...
class Device:
    ...
    def verify_block(self, block_to_verify, sending_miner):
        ...
        if self.check_signature:
            ...
            if not self.verify_msg_xmss(hash, signature, pub_key) and not self.verify_dilithium(pub_key, mined_by,
                                                                                                tree_no):
                print(
    ...
    def worker_local_update(self, rewards, log_files_folder_path_comm_round, comm_round, local_epochs=1):
        ...
        for epoch in range(local_epochs):
            ...
            for data, label in self.train_dl:
                ...
                self.local_updates_rewards_per_transaction += (
                        rewards * label.shape[0])
    ...
    def validate_model_weights1(self, weights_to_eval, deviceId, comm_round):
        with torch.no_grad():
            ...
            with open(f"{bench_folder}/validation.txt", "a") as file:
                file.write(
                    f"COMM ROUND: {comm_round} | Validation done by {self.return_idx()} for worker {deviceId}  which is  {str(self.devices_dict[deviceId].is_malicious)}\n")
                file.write(f"{losses}\n")
    ...
    def return_validations_and_signature(self, comm_round):
        ...
        validation_transaction_dict["validator_signature"] = self.sign_msg_xmss(
    ...
    def validate_worker_transaction(self, transaction_to_validate, rewards, log_files_folder_path, comm_round,
        ...
        if self.computation_power == 0:
            ...
            if self.check_signature:
                ...
                if self.verify_msg_xmss(hash, signature, pub_key) and self.verify_dilithium(pub_key, worker_device_id,
                                                                                            worker_tree_no):
                    print(
            ...
            if transaction_to_validate['worker_signature_valid']:
                ...
                accuracy_by_worker_update_using_own_data, losses = self.validate_model_weights1(
                    transaction_to_validate["local_updates_params"], worker_device_idd, comm_round)
